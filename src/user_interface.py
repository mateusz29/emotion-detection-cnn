...
class EmotionRecognitionApp(ctk.CTk):
    ...
    def _create_buttons(self: Self) -> None:
        ...
        # Select image button
        self.select_btn = ctk.CTkButton(self.button_frame, text="Select Image", command=self.load_image, width=button_width)
        self.select_btn.pack(pady=15)

        # Detect face button
        self.detect_btn = ctk.CTkButton(self.button_frame, text="Detect Face", command=self.detect_face, width=button_width)
        self.detect_btn.pack(pady=15)
        self.detect_btn.configure(state="disabled")

        # Predict emotion button
        self.predict_btn = ctk.CTkButton(self.button_frame, text="Predict Emotion", command=self.predict_emotion, width=button_width)
    ...
    def load_image(self: Self) -> None:
        ...
        if file_path:
            ...
            self.image = self.image.resize(self.image_size)  # Resize image to fixed size
    ...
    def detect_face(self: Self) -> None:
        if self.image:
            ...
            self.detected_faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=6)

            if len(self.detected_faces) > 0:
                for (x, y, w, h) in self.detected_faces:
                    cv2.rectangle(img_cv, (x, y), (x + w, y + h), (205, 245, 95), 2)  # Draw rectangle
    ...
    def predict_emotion(self: Self) -> None:
        if len(self.detected_faces) > 0:
            ...
            for (x, y, w, h) in self.detected_faces:
                face_img = Image.fromarray(cv2.cvtColor(img_cv[y:y + h, x:x + w], cv2.COLOR_BGR2RGB))
                ...
                draw.text((x, y - 36), emotion, fill=(202, 61, 245), font=ImageFont.truetype("arialbd.ttf", 28))
...
def main() -> None:
    ...
    model = CNN().to(device)
    model.load_state_dict(torch.load('./model/emotion_cnn.pth', map_location=device, weights_only=True))
    model.eval()
